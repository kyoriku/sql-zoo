...
SELECT party, votes, RANK() OVER(ORDER BY votes DESC) AS rank
  FROM ge
 WHERE constituency='S14000024' AND yr=2017
 ORDER BY party
...
-- 5. You can use SELECT within SELECT (https://sqlzoo.net/wiki/SELECT_within_SELECT_Tutorial) to pick out only the winners in Edinburgh.
-- Show the parties that won for each Edinburgh constituency in 2017.
SELECT constituency, party
  FROM (SELECT constituency, party, votes, RANK() OVER (PARTITION BY constituency ORDER BY votes DESC) as posn
          FROM ge
         WHERE constituency BETWEEN 'S14000021' AND 'S14000026' AND yr=2017) AS r
 WHERE posn = 1
...
SELECT party, COUNT(*) AS seats
  FROM (SELECT constituency, party, votes, RANK() OVER (PARTITION BY constituency ORDER BY votes DESC) as posn
          FROM ge
         WHERE constituency LIKE 'S%' AND yr=2017) AS r
 WHERE posn = 1
GROUP BY party
